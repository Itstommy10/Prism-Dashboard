...
// Supports: hass-anycubic_cloud integration (https://github.com/WaresWichall/hass-anycubic_cloud)
// For Anycubic Cloud printers (Kobra series with ACE multi-color box support)
...
const SUPPORTED_PRINTER_MODELS = [
  // Anycubic Kobra series (FDM) - supported by hass-anycubic_cloud
  'Anycubic Kobra X', 'Kobra X',
  'Anycubic Kobra 3', 'Kobra 3', 'Kobra 3 Max', 'Kobra 3 Combo',
  'Anycubic Kobra 2', 'Kobra 2', 'Kobra 2 Pro', 'Kobra 2 Plus', 'Kobra 2 Max', 'Kobra 2 Neo',
  'Anycubic Kobra', 'Kobra', 'Kobra Max', 'Kobra Plus', 'Kobra Neo', 'Kobra Go',
...
// Entity translation keys produced by hass-anycubic_cloud (entity.py has_entity_name = True)
// We match primarily by translation_key (== entity_description.key), which is collision-free.
const ENTITY_KEYS = [
  // Printer / job sensors
  'current_status',         // busy | available | unknown
  'job_state',              // printing | paused | finished | failed | downloading | ... | unknown
  'job_name',               // current file/job name
  'job_progress',           // Progress in %
  'job_time_remaining',     // Time remaining in MINUTES (int)
  'job_time_elapsed',       // Elapsed time in minutes
  'job_eta',                // ETA (timestamp device_class)
  'job_current_layer',      // Current layer
  'job_total_layers',       // Total layers
  // FDM temperatures / fan
  'curr_nozzle_temp',       // Current nozzle temp
  'target_nozzle_temp',     // Target nozzle temp
  'curr_hotbed_temp',       // Current bed temp
  'target_hotbed_temp',     // Target bed temp
  'fan_speed_pct',          // Fan speed %
  'print_speed_pct',        // Print speed %
  'print_speed_mode',       // Print speed mode (quiet/standard/sport/...)
  // Preview image (image domain)
  'job_image_url',          // Job/model preview thumbnail
  // ACE multi-color box
  'ace_spools',             // state active/inactive, attribute spool_info (list of slots)
  'ace_current_temperature',// ACE internal temperature
  'dry_status_remaining_time',
  'dry_status_target_temperature',
  'dry_status_is_drying',
  // Binary sensors (status)
  'job_in_progress', 'job_paused', 'job_complete', 'job_failed',
  'printer_online', 'is_busy', 'is_available',
  // Control buttons
  'pause_print', 'resume_print', 'cancel_print'
];

class PrismAnycubicCard extends HTMLElement {
  constructor() {
    super();
  ...
  static getConfigForm() {
    ...
    // Support hass-anycubic_cloud integration
    ...
    // Integration-based filter (catches devices by integration name)
    printerFilterCombinations.push({ integration: 'anycubic_cloud' });  // Anycubic Cloud

    return {
      schema: [
        {
          name: 'printer',
          label: 'Printer Device (Anycubic Cloud)',
          required: true,
        ...
        {
          name: 'camera_entity',
          label: 'Camera entity (optional - not provided by Anycubic Cloud)',
          selector: { entity: { domain: 'camera' } }
        },
        {
          name: 'light_switch',
          label: 'Light switch entity (optional - e.g. a smart plug or LED switch)',
        ...
        },
        {
          ...
          label: 'Cover image vertical position (% from bottom - align to print bed)',
          default: 29,
          selector: { number: { min: 0, max: 90, step: 1, mode: 'slider', unit_of_measurement: '%' } }
        },
        {
          name: 'cover_image_position_h',
          label: 'Cover image horizontal position (% from left)',
          default: 50,
          selector: { number: { min: 0, max: 100, step: 1, mode: 'slider', unit_of_measurement: '%' } }
        },
        {
          name: 'cover_image_opacity',
          label: 'Cover image opacity',
          default: 100,
          selector: { number: { min: 0, max: 100, step: 1, mode: 'slider', unit_of_measurement: '%' } }
        },
        {
          name: 'printer_image_opacity',
          label: 'Printer image opacity',
          default: 100,
          selector: { number: { min: 0, max: 100, step: 1, mode: 'slider', unit_of_measurement: '%' } }
        },
        {
        ...
        // Display Options section - toggle visibility of chips/overlays
        {
          ...
          schema: [
            ...
            {
              name: 'show_filament_percent',
              label: 'Show Filament Percentage (manual, tap filament in popup to set)',
              default: true,
              selector: { boolean: {} }
            },
            {
              name: 'show_cover_progress_percent',
              label: 'Show Progress % badge on the model preview image',
              default: true,
              selector: { boolean: {} }
            }
          ]
        },
        // ACE (Anycubic Color Engine / multi-color box) section
        {
          type: 'expandable',
          name: '',
          title: 'ACE (Anycubic Color Engine)',
          schema: [
            {
              name: 'show_cfs',
              label: 'Show ACE filament slots (auto-detected from ace_spools)',
              default: true,
              selector: { boolean: {} }
            },
            {
              name: 'show_cfs_info',
              label: 'Show ACE Temperature',
              default: true,
              selector: { boolean: {} }
            },
            {
  ...
  getAnycubicDeviceEntities() {
    if (!this._hass || !this.config?.printer) return {};

    const deviceId = this.config.printer;
    const result = {};

    // Support the Anycubic Cloud platform
    const supportedPlatforms = ['anycubic_cloud', 'anycubic'];
  ...
  getPrinterDataForDevice(deviceId, customCameraEntity, customName) {
    ...
    // Resolve an entity by translation_key for this device, with entity_id-suffix fallback
    const DE = (key, fallbackPattern = null, domain = null) => {
      const direct = deviceEntities[key]?.entity_id;
      if (direct) return direct;
      if (fallbackPattern) return this.findEntityByPatternForDevice(deviceId, fallbackPattern, domain);
      return null;
    };
    const stateOf = (entityId) => (entityId ? this._hass.states[entityId]?.state : null);
    const isOnDev = (entityId) => stateOf(entityId) === 'on';

    // Progress
    let progress = parseFloat(stateOf(DE('job_progress', 'job_progress', 'sensor'))) || 0;

    // Raw states
    const jobStateRaw = (stateOf(DE('job_state', 'job_state', 'sensor')) || '').toLowerCase();
    const currentStatusRaw = (stateOf(DE('current_status', 'current_status', 'sensor')) || '').toLowerCase();
    const onlineState = stateOf(DE('printer_online', 'printer_online', 'binary_sensor'));
    const isOffline = onlineState === 'off' ||
                      ((!onlineState || onlineState === 'unavailable') && currentStatusRaw === 'unknown' && !jobStateRaw);

    const activeJobStates = ['printing', 'downloading', 'checking', 'preheating', 'slicing'];
    let isPaused = isOnDev(DE('job_paused', 'job_paused', 'binary_sensor')) || jobStateRaw === 'paused';
    let isPrinting = !isPaused && (isOnDev(DE('job_in_progress', 'job_in_progress', 'binary_sensor')) || activeJobStates.includes(jobStateRaw));

    const statusDisplayMap = {
      printing: 'Printing', paused: 'Paused', finished: 'Idle', failed: 'Error',
      downloading: 'Downloading', checking: 'Checking', preheating: 'Preheating',
      slicing: 'Slicing', unknown: 'Idle', '': 'Idle'
    };
    let stateStr;
    if (isOffline) {
      stateStr = 'Offline';
      ...
      stateStr = statusDisplayMap[jobStateRaw] || 'Printing';
    } else {
      stateStr = jobStateRaw === 'failed' ? 'Error' : (statusDisplayMap[jobStateRaw] ?? 'Idle');
    }
    const isIdle = !isPrinting && !isPaused;

    // Remaining time - job_time_remaining is in MINUTES (integer)
    let printTimeLeft = '--';
    const timeEntity = DE('job_time_remaining', 'time_remaining', 'sensor');
    if (timeEntity && (isPrinting || isPaused)) {
      const timeValue = parseFloat(stateOf(timeEntity)) || 0;
      if (timeValue > 0) {
    ...
    if (isPrinting || isPaused) {
      currentLayer = parseInt(stateOf(DE('job_current_layer', 'current_layer', 'sensor'))) || 0;
      totalLayers = parseInt(stateOf(DE('job_total_layers', 'total_layers', 'sensor'))) || 0;
    }

    // Temperatures (no chamber sensor on Anycubic Cloud)
    let nozzleTemp = 0, targetNozzleTemp = 0, bedTemp = 0, targetBedTemp = 0, chamberTemp = 0;
    nozzleTemp = parseFloat(stateOf(DE('curr_nozzle_temp'))) || 0;
    targetNozzleTemp = parseFloat(stateOf(DE('target_nozzle_temp', 'target_nozzle_temperature', 'sensor'))) || 0;
    bedTemp = parseFloat(stateOf(DE('curr_hotbed_temp'))) || 0;
    targetBedTemp = parseFloat(stateOf(DE('target_hotbed_temp', 'target_hotbed_temperature', 'sensor'))) || 0;

    // Camera entity (only user-configured - integration provides none)
    ...
    // Device name
    const device = this._hass.devices?.[deviceId];
    const name = customName || device?.name || 'Anycubic Printer';
  ...
  set hass(hass) {
    ...
    if (this.config?.printer && (firstTime || Object.keys(this._deviceEntities).length === 0)) {
      this._deviceEntities = this.getAnycubicDeviceEntities();
    }
  ...
  updateValues() {
    ...
    // Update job info panel (above controls)
    const jobNameEl = this.shadowRoot.querySelector('[data-field="job-name"]');
    if (jobNameEl) {
      const jobNameVal = data.fileName || '--';
      if (jobNameEl.textContent !== jobNameVal) jobNameEl.textContent = jobNameVal;
      jobNameEl.title = data.fileName || '';
    }
    updateText('[data-field="job-state"]', data.stateStr);
    updateText('[data-field="job-progress"]', `${Math.round(data.progress)}%`);
    updateText('[data-field="job-time"]', data.printTimeLeft);
    updateText('[data-field="job-elapsed"]', data.printElapsed);
    updateText('[data-field="job-speed"]', data.speedMode);
    updateText('[data-field="job-date"]', data.jobDate);
  ...
  setupListeners() {
    ...
    // Tap the filament color swatch or the Remaining value to set the % manually
    const popupColor = this.shadowRoot?.querySelector('.filament-popup-color');
    if (popupColor) {
      popupColor.onclick = (e) => { e.stopPropagation(); this._editFilamentPercent(); };
    }
    const popupRemaining = this.shadowRoot?.querySelector('.filament-stat-remaining');
    if (popupRemaining) {
      popupRemaining.onclick = (e) => { e.stopPropagation(); this._editFilamentPercent(); };
    }
  }
  ...
  openFilamentPopup(slotElement) {
    ...
    // Track which slot the popup is editing (for manual percentage input)
    this._currentPopupSlotId = slotId;
    const remainEnabled = slotElement.dataset.remainEnabled === 'true';
    ...
    if (remainingEl) remainingEl.textContent = remainEnabled && remaining >= 0 ? `${remaining}%` : 'Tap to set';
  ...
  // Prompt the user to manually enter the filament percentage for the open slot
  _editFilamentPercent() {
    const slotId = this._currentPopupSlotId;
    if (!slotId) return;
    const current = this._getFilamentPercent(slotId);
    const input = window.prompt('Filament percentage (0-100, empty to clear):', current != null ? String(current) : '');
    if (input === null) return; // cancelled
    const trimmed = input.trim();
    if (trimmed === '') {
      this._setFilamentPercent(slotId, null);
    } else {
      let pct = parseFloat(trimmed);
      if (isNaN(pct)) return;
      pct = Math.max(0, Math.min(100, Math.round(pct)));
      this._setFilamentPercent(slotId, pct);
    }
    this.closeFilamentPopup();
  }

  // ---- Manual filament percentage storage (per printer / per slot) ----
  _getFilamentPercentStorageKey() {
    return `prism-anycubic-filament-pct-${this.config?.printer || 'default'}`;
  }
  _getFilamentPercents() {
    try {
      return JSON.parse(localStorage.getItem(this._getFilamentPercentStorageKey()) || '{}') || {};
    } catch (e) {
      return {};
    }
  }
  _getFilamentPercent(slotId) {
    const map = this._getFilamentPercents();
    const val = map[slotId];
    return (typeof val === 'number' && !isNaN(val)) ? val : null;
  }
  _setFilamentPercent(slotId, pct) {
    const map = this._getFilamentPercents();
    if (pct == null) {
      delete map[slotId];
    } else {
    ...
    localStorage.setItem(this._getFilamentPercentStorageKey(), JSON.stringify(map));
    this.render();
    this.setupListeners();
  }
  ...
  _getSpoolmanStorageKey() {
    return `prism-anycubic-spoolman-${this.config?.printer || 'default'}`;
  }
  ...
  sendNotification(message, title, data = {}) {
    ...
    const printerName = this.config.name || 'Anycubic Printer';
  ...
  openCameraPopup() {
    ...
    const deviceId = this.config.printer;
    const device = this._hass.devices?.[deviceId];
    const printerName = this.config.name || device?.name || 'Anycubic Printer';
  ...
  getPrinterData() {
    ...
    // Find entities for the hass-anycubic_cloud integration.
    // Primary resolution is by translation_key (== entity_description.key), which is
    // collision-free. We fall back to an entity_id-suffix pattern search if needed.
    const E = (key, fallbackPattern = null, domain = null) => {
      const direct = this._deviceEntities[key]?.entity_id;
      if (direct) return direct;
      if (fallbackPattern) return this.findEntityByPattern(fallbackPattern, domain);
      return null;
    };

    // Status / progress / layers / time
    const progressEntity = E('job_progress', 'job_progress', 'sensor');
    const jobStateEntity = E('job_state', 'job_state', 'sensor');
    const currentStatusEntity = E('current_status', 'current_status', 'sensor');
    const layerEntity = E('job_current_layer', 'current_layer', 'sensor');
    const totalLayerEntity = E('job_total_layers', 'total_layers', 'sensor');
    // job_time_remaining is reported in MINUTES (integer)
    const timeLeftEntity = E('job_time_remaining', 'time_remaining', 'sensor');

    // Binary sensors used for reliable state detection
    const jobInProgressEntity = E('job_in_progress', 'job_in_progress', 'binary_sensor');
    const jobPausedEntity = E('job_paused', 'job_paused', 'binary_sensor');
    const jobCompleteEntity = E('job_complete', 'job_complete', 'binary_sensor');
    const jobFailedEntity = E('job_failed', 'job_failed', 'binary_sensor');
    const printerOnlineEntity = E('printer_online', 'printer_online', 'binary_sensor');

    // Temperatures (no chamber sensor on Anycubic Cloud)
    const nozzleTempEntity = E('curr_nozzle_temp', null);
    const targetNozzleTempEntity = E('target_nozzle_temp', 'target_nozzle_temperature', 'sensor');
    const bedTempEntity = E('curr_hotbed_temp', null);
    const targetBedTempEntity = E('target_hotbed_temp', 'target_hotbed_temperature', 'sensor');
    const boxTempEntity = null; // Anycubic Cloud exposes no chamber/enclosure temperature

    // Fan - only a single fan speed (% sensor) is exposed
    const modelFanEntity = E('fan_speed_pct', null);
    const caseFanEntity = null;
    const auxFanEntity = null;

    // Light: Anycubic Cloud provides no light entity - only a user-configured switch
    const lightSwitchEntity = null;
    const lightSensorEntity = null;

    // Camera: not provided by the integration - only a user-configured camera entity
    const cameraEntityAuto = null;

    // Job file name
    const fileNameEntity = E('job_name', 'job_name', 'sensor');

    // Elapsed time (minutes) and print speed mode
    const timeElapsedEntity = E('job_time_elapsed', 'time_elapsed', 'sensor');
    const speedModeEntity = E('print_speed_mode', 'speed_mode', 'select') || E('print_speed_pct', 'print_speed', 'sensor');

    // Thumbnail / cover image: the integration exposes image.<printer>_job_preview
    const thumbnailEntityAuto = E('job_image_url', 'job_preview', 'image');

    // Read values
    const progress = this.getEntityValueById(progressEntity);
    // Raw states from hass-anycubic_cloud
    // job_state ∈ {printing, paused, finished, failed, downloading, checking, preheating, slicing, unknown}
    // current_status ∈ {busy, available, unknown}
    const jobStateRaw = (this.getEntityStateById(jobStateEntity) || '').toLowerCase();
    const currentStatusRaw = (this.getEntityStateById(currentStatusEntity) || '').toLowerCase();
    ...
    // Offline detection via printer_online binary_sensor / current_status
    const onlineState = printerOnlineEntity ? this._hass.states[printerOnlineEntity]?.state : null;
    const isOffline = onlineState === 'off' ||
                      ((!onlineState || onlineState === 'unavailable') && currentStatusRaw === 'unknown' && !jobStateRaw);

    // Determine print state - prefer binary sensors, fall back to job_state string
    const activeJobStates = ['printing', 'downloading', 'checking', 'preheating', 'slicing'];
    let isPaused = isOn(jobPausedEntity) || jobStateRaw === 'paused';
    let isPrinting = !isPaused && (isOn(jobInProgressEntity) || activeJobStates.includes(jobStateRaw));
    let isIdle = !isPrinting && !isPaused;
    ...
    if (isOffline) {
      stateStr = 'Offline';
      isPrinting = false; isPaused = false; isIdle = true;
    } else if (isPaused) {
      stateStr = 'Paused';
    } else if (isPrinting) {
      stateStr = statusDisplayMap[jobStateRaw] || 'Printing';
    } else {
      stateStr = statusDisplayMap[jobStateRaw] ?? 'Idle';
      if (jobStateRaw === 'failed') stateStr = 'Error';
    }

    const statusLower = stateStr.toLowerCase();

    // Get remaining time - job_time_remaining is reported in MINUTES (integer)
    let printTimeLeft = '--';
    let printEndTime = '--:--';
    if (timeLeftEntity && (isPrinting || isPaused)) {
      const timeValue = this._hass.states[timeLeftEntity]?.state;
      if (timeValue && timeValue !== 'unknown' && timeValue !== 'unavailable' && timeValue !== 'none') {
        const totalMinutes = parseFloat(timeValue) || 0;
        if (totalMinutes > 0) {
    ...
    // Elapsed time - job_time_elapsed is reported in MINUTES
    let printElapsed = '--';
    if (timeElapsedEntity && (isPrinting || isPaused)) {
      const elapsedVal = parseFloat(this.getEntityStateById(timeElapsedEntity));
      if (!isNaN(elapsedVal) && elapsedVal >= 0) {
        const h = Math.floor(elapsedVal / 60);
        const m = Math.round(elapsedVal % 60);
        printElapsed = h > 0 ? `${h}h ${m}m` : `${m}m`;
      }
    }

    // Print speed mode (select like "standard"/"sport", or a % sensor)
    let speedMode = '--';
    if (speedModeEntity) {
      const smRaw = this.getEntityStateById(speedModeEntity);
      if (smRaw && !['unknown', 'unavailable', 'none', ''].includes(String(smRaw).toLowerCase())) {
        speedMode = speedModeEntity.split('.')[0] === 'sensor' && !isNaN(parseFloat(smRaw))
          ? `${Math.round(parseFloat(smRaw))}%`
          : String(smRaw).charAt(0).toUpperCase() + String(smRaw).slice(1);
      }
    }
    ...
    const deviceId = this.config.printer;
    const device = this._hass.devices?.[deviceId];
    const name = this.config.name || device?.name || 'Anycubic Printer';
    ...
    if (coverImageEntity) {
      // The image.<printer>_job_preview state is a timestamp of the last update -> job date
      const previewState = this._hass.states[coverImageEntity];
      const ts = previewState?.state;
      if (ts && !['unknown', 'unavailable', 'none', ''].includes(String(ts).toLowerCase())) {
        const d = new Date(ts);
        if (!isNaN(d.getTime())) {
          jobDate = d.toLocaleDateString([], { day: '2-digit', month: '2-digit', year: '2-digit' });
        }
      }
    }
    ...
    // ACE (Anycubic Color Engine / multi-color box) detection.
    // The integration exposes a single sensor.<printer>_ace_spools whose state is
    // "active"/"inactive" and whose attribute `spool_info` is a list of slots:
    //   { material_type: "PLA", color: [r,g,b], status: <int>, spool_loaded: <bool> }
    // There is no remaining-percentage field, so remainEnabled is false.
    let cfsData = [];
    let cfsTemperature = null;
    let cfsHumidity = null;        // Not exposed as an entity by hass-anycubic_cloud
    let externalSpoolData = null;  // Anycubic Cloud has no external-spool entities

    if (this.config.show_cfs !== false) {
      // ACE internal temperature
      const aceTempEntity = E('ace_current_temperature', 'ace_current_temperature', 'sensor');
      if (aceTempEntity) {
        const t = parseFloat(this._hass.states[aceTempEntity]?.state);
        if (!isNaN(t)) cfsTemperature = t;
      }

      const aceSpoolsEntity = E('ace_spools', 'ace_spools', 'sensor');
      const spoolInfo = aceSpoolsEntity
        ? this._hass.states[aceSpoolsEntity]?.attributes?.spool_info
        : null;

      if (Array.isArray(spoolInfo)) {
        const rgbToHex = (c) => {
          if (!Array.isArray(c) || c.length < 3) return null;
          return '#' + c.slice(0, 3).map((v) => {
            const h = Math.max(0, Math.min(255, parseInt(v) || 0)).toString(16);
            return h.length === 1 ? '0' + h : h;
          }).join('');
        };

        spoolInfo.forEach((slot, slotId) => {
          const filamentType = (slot?.material_type || '').toString();
          const filamentName = filamentType;
          // status === 5 means a spool is loaded in the slot (AnycubicSpoolInfo.spool_loaded)
          const hasSpool = slot?.spool_loaded === true || slot?.status === 5;
          const isSelected = slot?.spool_loaded === true;

          let color = rgbToHex(slot?.color) || '#666666';

          // Detect transparency by keyword in the material name
          let isTransparent = false;
          const transparencyKeywords = ['transparent', 'clear', 'translucent'];
          if (transparencyKeywords.some((kw) => filamentType.toLowerCase().includes(kw))) {
            isTransparent = true;
          }

          const isEmpty = !hasSpool || !filamentType ||
                          filamentType.toLowerCase() === 'unknown' ||
                          filamentType.toLowerCase() === 'empty';

          // Determine display type label
          let displayType = '';
          if (!isEmpty) {
            const typeMatch = filamentType.match(/\b(PETG|PLA|ABS|TPU|ASA|PA-CF|PA|PC|PVA|HIPS|PP)\b/i);
            if (typeMatch) {
              displayType = typeMatch[1].toUpperCase();
            } else if (filamentType.length <= 8) {
              displayType = filamentType.toUpperCase();
            } else {
              displayType = filamentType.substring(0, 6).toUpperCase();
            }
          }

          // ACE exposes no remaining percentage, but the user can set one manually
          const manualPct = this._getFilamentPercent(`0-${slotId}`);
          cfsData.push({
            id: `0-${slotId}`,
            boxId: 0,
            slotId,
            ...
            remaining: manualPct != null ? manualPct : 0,
            remainEnabled: manualPct != null,  // only show when manually set
            active: isSelected,
            empty: isEmpty,
    ...
    const returnData = {
      ...
      auxFanSpeed,
      caseFanSpeed,
      // Capability flags - Anycubic Cloud has no side/case fan nor chamber sensor
      hasAuxFan: !!auxFanEntity,
      hasCaseFan: !!caseFanEntity,
      hasChamber: !!boxTempEntity,
      currentLayer,
      totalLayers,
      name,
      ...
      // Cover image / Thumbnail - always show (also when idle, as "last print" preview)
      coverImageEntity,
      coverImageUrl,
      coverImagePosition: this.config.cover_image_position ?? 29,
      coverImagePositionH: this.config.cover_image_position_h ?? 50,
      coverImageOpacity: (this.config.cover_image_opacity ?? 100) / 100,
      printerImageOpacity: (this.config.printer_image_opacity ?? 100) / 100,
      showCoverImage: this.config.show_cover_image !== false && !!coverImageUrl,
      // Custom sensors
      humidity,
      customTemp,
      powerSwitch,
      ...
      // Display toggles for percentages
      showFilamentPercent: this.config.show_filament_percent !== false,
      showCoverProgressBadge: this.config.show_cover_progress_percent !== false,
  ...
  getPreviewData() {
    return {
      ...
      currentLayer: 12,
      totalLayers: 28,
      name: this.config?.name || 'Anycubic Printer',
      cameraEntity: null,
      cameraImage: null,
      ...
      // CFS preview data
      cfsData: [
        { id: '0-0', type: 'PLA', color: '#FF6B6B', remaining: 85, remainEnabled: true, active: true, empty: false, transparent: false, fullName: 'Creality PLA Red' },
        { id: '0-1', type: 'PETG', color: '#4ECDC4', remaining: 62, remainEnabled: true, active: false, empty: false, transparent: false, fullName: 'Creality PETG Teal' },
        { id: '0-2', type: 'ABS', color: '#FFE66D', remaining: 43, remainEnabled: true, active: false, empty: false, transparent: false, fullName: 'Creality ABS Yellow' },
        { id: '0-3', type: 'TPU', color: '#95E1D3', remaining: 91, remainEnabled: true, active: false, empty: false, transparent: true, fullName: 'Creality TPU Clear' }
      ],
      cfsTemperature: 28,
      cfsHumidity: null,
      ...
      showCfs: true,
      showCfsInfo: true,
      showFilamentPercent: true,
      showCoverProgressBadge: true,
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .cover-image-container {
            position: absolute;
            /* Vertical position is configurable (bottom %) to align with the print bed */
            left: 50%;
        ...
        /* Job status panel (above the control buttons) */
        .job-info {
            display: flex;
            flex-direction: column;
            gap: 8px;
      ...
      <div class="card ${(this.config?.layout_mode || 'auto') === 'wide' ? 'wide' : ''}">
        ...
        <div class="cfs-grid ${data.cfsData.length <= 3 ? 'slots-' + data.cfsData.length : ''} ${data.spoolView === 'front' ? 'front-view' : ''}">
            ${data.cfsData.map(slot => `
                <div class="cfs-slot ${slot.active ? 'active' : ''} ${!slot.empty ? 'clickable' : ''} ${slot.transparent ? 'transparent' : ''} ${data.spoolView === 'front' ? 'front-view' : ''}"
                     ...
                     data-remain-enabled="${slot.remainEnabled === true}"
                        ...
                        <div class="spool-front-wrapper">
                            ...
                            <div class="spool-front-filament ${slot.color === '#000000' ? 'dark-filament' : ''}" style="background-color: ${slot.color};">
                                ...
                                <div class="spool-front-label">
                                    ...
                                    ${data.showFilamentPercent && slot.remainEnabled && slot.remaining >= 0 ? `<span class="spool-front-label-weight">${slot.remaining}%</span>` : ''}
                                </div>
                            </div>
                    ...
                    <div class="spool-visual">
                        ...
                        ${data.showFilamentPercent && !slot.empty && slot.remainEnabled && slot.remaining >= 0 ? `<div class="remaining-badge">${slot.remaining}%</div>` : ''}
                    </div>
                    <div class="cfs-info">
                        <div class="cfs-type">${slot.empty ? 'Empty' : slot.type}</div>
                    </div>
        ...
        <div class="main-visual ${data.lightEntity && !data.isLightOn ? 'light-off' : ''}">
            ${data.powerSwitch ? `
            ...
            ` : `
                <img src="${data.printerImg}" class="printer-img ${data.lightEntity && !data.isLightOn ? 'dimmed' : ''}" style="opacity: ${data.printerImageOpacity};" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />
                <div class="printer-fallback-icon" style="display: none;">
                  <ha-icon icon="mdi:printer-3d"></ha-icon>
                </div>
                
                ${data.showCoverImage ? `
                <div class="cover-image-container" style="bottom: ${data.coverImagePosition}%; left: ${data.coverImagePositionH}%; opacity: ${data.coverImageOpacity};">
                    <div class="cover-image-wrapper ${data.isPrinting ? 'printing' : ''} ${data.isPaused ? 'paused' : ''} ${data.isIdle ? 'idle' : ''}">
                        ...
                        ${data.showCoverProgressBadge ? `<div class="cover-progress-badge">${Math.round(data.progress)}%</div>` : ''}
                    </div>
                </div>
                ` : ''}
                ...
                <div class="overlay-left">
                    ...
                    ` : ''}
                    ${this.config.show_aux_fan !== false && data.hasAuxFan ? `
                    <div class="overlay-pill">
                    ...
                    ` : ''}
                    ${this.config.show_case_fan !== false && data.hasCaseFan ? `
                    <div class="overlay-pill">
                ...
                <div class="overlay-right">
                    ...
                    ` : ''}
                    ${this.config.show_chamber_temp !== false && data.hasChamber ? `
                    <div class="overlay-pill right">
        ...
        <div class="job-info">
            <div class="job-info-name" data-field="job-name" title="${data.fileName || ''}">${data.fileName || '--'}</div>
            <div class="job-info-stats">
                <span class="job-chip"><ha-icon icon="mdi:information-outline"></ha-icon><span data-field="job-state">${data.stateStr}</span></span>
                <span class="job-chip"><ha-icon icon="mdi:progress-helper"></ha-icon><span data-field="job-progress">${Math.round(data.progress)}%</span></span>
                <span class="job-chip"><ha-icon icon="mdi:clock-outline"></ha-icon><span data-field="job-time">${data.printTimeLeft}</span></span>
                <span class="job-chip"><ha-icon icon="mdi:timer-sand"></ha-icon><span data-field="job-elapsed">${data.printElapsed}</span></span>
                <span class="job-chip"><ha-icon icon="mdi:speedometer"></ha-icon><span data-field="job-speed">${data.speedMode}</span></span>
                <span class="job-chip"><ha-icon icon="mdi:calendar"></ha-icon><span data-field="job-date">${data.jobDate}</span></span>
            </div>
        </div>
...
customElements.define('prism-anycubic', PrismAnycubicCard);
...
window.customCards.push({
  ...
  name: 'Prism Anycubic',
  preview: true,
  description: 'Anycubic 3D Printer card with ACE multi-color box support (hass-anycubic_cloud)'
});
